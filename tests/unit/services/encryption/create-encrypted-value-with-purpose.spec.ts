...
test.group(
  'It will test the encryption method using a random number between ' + min + ' and ' + max + ' ',
  async () => {
    test(
      'It creates a encrypted value that will receive this key: ' + randomNumber,
      ({ assert }) => {
    ...
    test(
      'It will decrypt a value that will only works using this key: ' + randomNumber,
      ({ assert }) => {
